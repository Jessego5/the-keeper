"""
These are the Tier 1 tests for the register (mood) classifier's structure. No key, no
model download: these drive mood.classify with a tiny fake embedder, so they assert the
RULES (floor, margin, neutral-as-no-signal) rather than any model's accuracy. Accuracy
